...
export function Modal({ title, description, open, onClose, children, headerIcon, overlayClassName, panelClassName }: ModalProps) {
    ...
    return (
        <div className={cn('fixed inset-0 z-50 flex items-end justify-center overflow-y-auto bg-slate-900/50 p-3 sm:items-center sm:p-4', overlayClassName)}>
            <div className={cn('w-full max-w-lg rounded-3xl bg-white p-5 shadow-2xl sm:p-6', panelClassName)}>
                {headerIcon ? (
                    <div className="mb-5 flex items-start justify-between gap-4 sm:mb-6">
                        ...
                        <button
                            type="button"
                            className="inline-flex shrink-0 items-center gap-2 rounded-2xl px-2.5 py-2 text-sm font-medium text-slate-500 transition hover:bg-slate-100 hover:text-slate-900 focus:outline-none focus:ring-2 focus:ring-purple-200"
                            onClick={onClose}
                        >
